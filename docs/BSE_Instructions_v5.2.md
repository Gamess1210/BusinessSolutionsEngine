...
The BSE is operated entirely by Comotion's hybrid BA team. There is no handoff to a separate developer. The same person who captures the client problem at Gate 1 also approves generated code at Gate 5 and signs off on deliverables at Gate 6.
...
| Specification | Hybrid BA | Gate 4 |
| Code Generation & Review | Hybrid BA | Gate 5 |
| Output & Delivery | Hybrid BA | Gate 6 |
...
│   At threshold (or max cycles): Gate 5 triggered                │
└─────────────────────┬────────────────────────────────────────────┘
                      │
                      ▼
┌──────────────────────────────────────────────────────────────────┐
...
Capture inputs (any combination)
  ...
  Client Intake Form   ──┘
                    ...
                    approved brief + solutions → CONTEXT.md
                    committed to client repo branch
                              │
                              ▼
                    ...
                    Spec stored in repo — not Supabase
                              │
                    ┌─────────▼──────────────────────────────────┐
                    │  GATE 4 — Spec Approval         [v5.1]     │ ◀── BA reviews OpenSpec
                    ...
                    └─────────┬────────────────────────────────────────┘
                              │ [Gate 5 triggered]
                    ┌─────────▼──────────────────┐
                    │  GATE 5 — Code Review       │ ◀── BA reviews Gemini
                    ...
                    → SharePoint via Graph API
                              │
                    ┌─────────▼──────────────────────┐
                    │  GATE 6 — Output Review         │ ◀── BA previews
...
| contextGenerationChain | chains/contextGeneration.js | Extracts domain vocabulary from brief + solutions → CONTEXT.md |
...
| outputGenerationChain | chains/outputGeneration.js | Generates final A4 HTML documents after Gate 5 approval |
...
- `/review/[id]/outputs` — Gate 6 output preview and final approval. Client documents only — no scorecard.
...
Gate 3 is the client-facing proposal gate. It sits between solutions approval (Gate 2) and specification generation (Gate 4). Its purpose is to give the BA a reviewed, Comotion-branded business proposal to send to the client before any technical work begins.
...
**Architecture note:** Gate 3 does not block Gate 4. Gate 4 (spec generation) begins only after Gate 3 is fully approved and sent. The proposal must be in the client's hands before technical specification begins.

### 7.4 Gate 4 — Spec Review Detail

Gate 4 now reviews OpenSpec files committed to the client repo, not a JSON object stored in Supabase. The BA approves the exact files Claude Code reads. There is no translation step between what is approved and what drives code generation.

Gate 4 is the most consequential technical gate. All code generation is downstream of what is approved here.

**What happens before Gate 4:**

After Gate 3 approval, two chains run in sequence:

`contextGenerationChain` — Claude reads the approved brief and solutions and produces `CONTEXT.md` (domain vocabulary, key terms, architectural decisions) and `docs/adr/` entries for the client repo. These files are committed to the `feature/{client-name}/{engagement-id}` branch. This replaces the v4 `/grill-with-docs` approach — domain context is extracted automatically from the structured data already approved by the BA, with no interactive session required.

`openspecGenerationChain` — Claude generates the full technical specification in OpenSpec format. Epics become capability folders under `openspec/changes/{engagement-id}/specs/`. User stories become `### Requirement:` headers with `#### Scenario:` blocks in WHEN/THEN/AND syntax. Acceptance criteria become scenario steps. Files are committed to the client repo branch. The `specifications` table in Supabase stores `repo_path` and `commit_sha` — not the spec content itself.

**What the BA sees at Gate 4:**
...
4. Gate 4 review screen reloads with regenerated sections highlighted for re-review
...
**Client repo setup (runs once after Gate 4 approval):**
...
2. ESLint complexity rule (no API cost) — produces per-file CC scores:
   ...
   - CC 21+: untestable — loop pauses. BA notified via Teams with file list and CC scores. BA approves continuation or rejects for refactor. Pipeline does not proceed until explicit BA decision recorded in `gate_approvals` (`gate_number = 5`, `action = 'cc_pause_approved'` or `'cc_pause_rejected'`).
...
`/diagnose` is available to the BA as a manual tool after Gate 5 for deep investigation of specific issues before deciding to approve or reject. It is not used in the automated loop.
...
→ proposal_pending → gate3_review → spec_pending → gate4_review
...
### 7.7 Human-in-the-Loop Gates [v5.1 CHANGE]

Six mandatory approval gates. Status enforced server-side in every Vercel API route. Frontend only reflects state — never controls it.
...
- **Final Client Brief PDF** — generated at Gate 6 for formal post-engagement delivery. Client-facing. No scorecard data.
...
| Gate 6 approved | `gate_approvals`: `gate_number = 6`, `action = 'approved'` | Teams + email confirmation with SharePoint links |
| Gate 6 rejected | `gate_approvals`: `gate_number = 6`, `action = 'rejected'` | Teams + email alert to BA |
...
**Schema additions first:** [v5.1 CHANGE]
...
- Extend `engagements.status` with `'proposal_pending'`, `'gate3_review'`, `'gate4_review'`, `'gate5_review'`, `'gate6_review'`, `'failed'` states [v5.1]
- Extend `gate_approvals.gate_number` check to cover `1–6` [v5.1]
...
- Implement Gate 4 spec review screen — renders OpenSpec markdown files; sections flaggable for regeneration
...
- Build Gate 5 code review screen — Gemini scorecard, ESLint CC scores per file (colour-coded), history, diff view, escalation flag [v5.1]
...
- Implement `openspec archive` step (runs after Gate 5 approval, before output generation)
...
- Build Gate 6 review screen — client document preview, approve/reject
- Wire Power Automate Gate 6 flows (approval and rejection)
- End-to-end test including document generation and SharePoint filing

### Phase 5 — Power Platform & Refinement

- Build all Power Automate flows (intake notification, proposal send, CC pause notification, review report delivery, Gate 6 approval/rejection, chain failure alert) [v5.1]
...
### Gate Numbering Reference [v5.1 CHANGE]

```
Gate 1 — Brief Review
Gate 2 — Solutions Review
Gate 3 — Business Proposal (NEW)
Gate 4 — Spec Approval (was Gate 3)
Gate 5 — Code Review (was Gate 4)
Gate 6 — Output Review (was Gate 5)
```
...
React naming conventions. Tailwind class ordering. Supabase query patterns (`supabase-js` v2). Error handling and recovery pattern. Gate enforcement pattern (always server-side). Gate numbering reference (Gates 1–6). Feature flag pattern. Model separation rule (Claude generates and fixes, Gemini reviews — never swap). Non-pipeline AI call labelling convention. OpenSpec file structure and WHEN/THEN/AND format. Fallow hook setup pattern. ESLint complexity rule configuration and CC severity classification. CC pause pattern.

**Skill 6 — OpenSpec + Matt Pocock Skills Reference**
OpenSpec BSE-custom schema definition. CLI commands used by BSE (`init`, `validate --all --json`, `archive`, `schema init`). Matt Pocock skills used in pipeline and their setup order: `/setup-matt-pocock-skills` (must run before `/to-issues`), `/git-guardrails`, `/to-issues`, `/tdd`, `/caveman`, `/zoom-out`. `/diagnose` as manual post-Gate-5 tool only — not in automated loop. Fallow hook installation and `gate=new-only` behaviour.
