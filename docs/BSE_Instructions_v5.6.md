# Business Solutions Engine (BSE) — Full Project Instructions v5.6
...
4. On approval: a `gate_approvals` record is inserted (`gate_number = 5`, `action = 'plan_approved'`). Status advances to `spec_pending`. The approved project plan is stored in `engagements.project_plan` (JSONB). The full question/answer conversation history is stored in `engagements.plan_conversation` (JSONB).
...
- `POST /api/pipeline/gate5-approve` — validates `engagement.status === 'plan_pending'` (409 otherwise); inserts `gate_approvals` record (`gate_number: 5`, `action: 'plan_approved'`); stores plan in `engagements.project_plan`; advances status to `spec_pending`
