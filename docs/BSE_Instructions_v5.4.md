# Business Solutions Engine (BSE) — Full Project Instructions v5.4

**How to use this document** This is the complete operating manual for the BSE build project. You are a Claude instance in a dedicated build project. Your job is to implement the system described here. Read this document in full before writing any code. When in doubt, refer back here. This document is self-contained and supersedes all previous versions (v1.0, v2.0, v3.0, v4.0, v5.0, v5.1, v5.2, v5.3) and all prior pipeline planning notes.

**What changed from v5.3** Sections 2, 5.2, 6.2, 7.1, 7.7, 9, 15, 16, 17 updated to reflect 7-gate pipeline with new Gate 4 Client Decision and Context. Section 7.4 renamed to Gate 5 Spec Review Detail with internal references updated. All existing gates from Gate 4 onwards renumbered: former Gate 4 (Spec Approval) is now Gate 5, former Gate 5 (Code Review) is now Gate 6, former Gate 6 (Output Review) is now Gate 7. Affected sections are marked [v5.4]. See the change log at the end of this document for a full summary.
...
The BSE is operated entirely by Comotion's hybrid BA team. There is no handoff to a separate developer. The same person who captures the client problem at Gate 1 also approves generated code at Gate 6 and signs off on deliverables at Gate 7.
...
| Specification | Hybrid BA | Gate 5 |
| Code Generation & Review | Hybrid BA | Gate 6 |
| Output & Delivery | Hybrid BA | Gate 7 |
...
Capture inputs (any combination)
  ...
  Client Intake Form   ──┘
                    ...
                    WHEN/THEN/AND format using chosen solution
                    and all engagement inputs
                    ...
                    └─────────┬────────────────────────────────────────┘
                              │ [Gate 6 triggered]
                    ┌─────────▼──────────────────┐
                    │  GATE 6 — Code Review       │ ◀── BA reviews Gemini
                    ...
                    → SharePoint via Graph API
                              │
                    ┌─────────▼──────────────────────┐
                    │  GATE 7 — Output Review         │ ◀── BA previews
...
### 6.2 Chains to Implement [v5.1 CHANGE]
...
**Gate 4 — no AI chain:** Gate 4 (Client Decision and Context) is pure human input capture. No AI chain runs at this gate. `contextGenerationChain` and `openspecGenerationChain` run after Gate 4 approval, immediately before Gate 5. [v5.4]
...
| proposalGenerationChain | chains/proposalGeneration.js | Generates A4 HTML business proposal PDF from brief + solutions |
...
| openspecGenerationChain | chains/openspecGeneration.js | Generates spec in OpenSpec WHEN/THEN/AND format; commits to client repo |
...
| outputGenerationChain | chains/outputGeneration.js | Generates final A4 HTML documents after Gate 6 approval |
...
ESLint complexity scoring is now added to the pre-check stage. A CC score of 21+ pauses the loop for human decision rather than blocking outright. The Gate 6 review screen displays per-file CC scores alongside the Gemini scorecard.
...
reviewLoopChain:
  ...
  5. Check: all 5 dimension scores ≥ threshold?
     YES → trigger Gate 6 (human approval)
     NO  → enter fix loop:
  ...
  6. Max iterations: REVIEW_MAX_CYCLES (default: 5, env var)
     If max reached without threshold:
       → trigger Gate 6 with review_escalated = true
       → flag prominently in Gate 6 UI
...
- Total time taken (loop start to Gate 6 trigger)
...
ESLint complexity scoring runs independently in pre-check and is displayed separately on the Gate 6 screen — it is not one of Gemini's five dimensions. The two signals are complementary: ESLint gives a per-file objective CC score; Gemini gives a holistic subjective complexity assessment across the full codebase.
...
- `/review/[id]/client-decision` — Gate 4 client decision. BA selects chosen solution from approved options (required, radio button selection), adds supplementary context via brain-dump, transcript, or guided questions (all optional), or checks no-further-input. Advances to spec_pending on approval. [v5.4 NEW]
- `/review/[id]/spec` — Gate 5 spec review. Renders OpenSpec markdown files. Sections flaggable for regeneration.
- `/review/[id]/code` — Gate 6 code review. Gemini scorecard, ESLint CC scores per file, per-cycle review history, diff view, escalation flag. [v5.1 CHANGE]
- `/review/[id]/outputs` — Gate 7 output preview and final approval. Client documents only — no scorecard.
...
### 7.3 Gate 3 — Business Proposal Review [v5.1 NEW]

Gate 3 is the client-facing proposal gate. It sits between solutions approval (Gate 2) and the client decision step (Gate 4). Its purpose is to give the BA a reviewed, Comotion-branded business proposal to send to the client before any technical work begins.

**What happens before Gate 3:**

After Gate 2 approval, `proposalGenerationChain` runs:
...
**What the BA sees at Gate 3:**
- Embedded PDF preview of the Comotion-branded business proposal
- Approve or reject controls
- On approval: a "Send to Client" button becomes active
- The BA clicks Send — a Power Automate flow fires, delivering the PDF to `engagements.client_email` via Outlook email with a Comotion-branded email wrapper
- Gate 3 is not complete until both approval AND send have occurred

**Proposal document contents:**
- Comotion branding throughout (logo placeholder until brand assets supplied)
- Executive summary
- Problem statement
- Stakeholder impact summary
- Solution options (title, description, effort, impact, key risk per option)
- Next steps / recommended path forward
- No internal pipeline data, no scores, no technical spec content

**Rejection flow:**
- BA rejects → `engagement.status` → `solutions_pending`
- BA edits solutions at Gate 2 and re-approves
- `proposalGenerationChain` reruns with updated solutions
- New PDF generated, Gate 3 re-triggered

**Architecture note:** Gate 3 does not block Gate 4. Gate 4 (client decision) begins only after Gate 3 is fully approved and sent. The proposal must be in the client's hands before the BA records the solution decision.

### 7.4 Gate 5 — Spec Review Detail

Gate 5 now reviews OpenSpec files committed to the client repo, not a JSON object stored in Supabase. The BA approves the exact files Claude Code reads. There is no translation step between what is approved and what drives code generation.

Gate 5 is the most consequential technical gate. All code generation is downstream of what is approved here.

**What happens before Gate 5:**

After Gate 4 approval, two chains run in sequence:

`contextGenerationChain` — Claude reads the chosen solution, approved brief, and all engagement inputs and produces `CONTEXT.md` (domain vocabulary, key terms, architectural decisions) and `docs/adr/` entries for the client repo. These files are committed to the `feature/{client-name}/{engagement-id}` branch. This replaces the v4 `/grill-with-docs` approach — domain context is extracted automatically from the structured data already approved by the BA, with no interactive session required.

`openspecGenerationChain` — Claude generates the full technical specification in OpenSpec format using the chosen solution and all engagement inputs. Epics become capability folders under `openspec/changes/{engagement-id}/specs/`. User stories become `### Requirement:` headers with `#### Scenario:` blocks in WHEN/THEN/AND syntax. Acceptance criteria become scenario steps. Files are committed to the client repo branch. The `specifications` table in Supabase stores `repo_path` and `commit_sha` — not the spec content itself.

**What the BA sees at Gate 5:**
...
4. Gate 5 review screen reloads with regenerated sections highlighted for re-review
5. BA approves or flags again until satisfied

**Approval:** Commits final spec state to branch. Updates `specifications` table with `commit_sha` and `approved_at`. `engagement.status` → `code_pending`.
...
**Client repo setup (runs once after Gate 5 approval):**
...
2. ESLint complexity rule (no API cost) — produces per-file CC scores:
   ...
   - CC 21+: untestable — loop pauses. BA notified via Teams with file list and CC scores. BA approves continuation or rejects for refactor. Pipeline does not proceed until explicit BA decision recorded in `gate_approvals` (`gate_number = 6`, `action = 'cc_pause_approved'` or `'cc_pause_rejected'`).
...
`/diagnose` is available to the BA as a manual tool after Gate 6 for deep investigation of specific issues before deciding to approve or reject. It is not used in the automated loop.
...
### 7.7 Human-in-the-Loop Gates [v5.1 CHANGE] [v5.4 CHANGE]

Seven mandatory approval gates. Status enforced server-side in every Vercel API route. Frontend only reflects state — never controls it.
...
| Gate 4 — Client Decision and Context | Gate 3 approved and sent | Select chosen solution from approved options (required); add supplementary context via brain-dump, transcript, or guided questions (all optional); or check no-further-input if no additional context | None |
| Gate 5 — Spec Approval | OpenSpec files committed to client repo | Review OpenSpec markdown; flag sections for regeneration; approve | None |
| Gate 6 — Code Review | Review loop completes (threshold or max cycles) | Review Gemini scorecard, ESLint CC scores, cycle history, code diff; approve or reject. `/diagnose` available manually. | Teams: CC 21+ pause events only |
| Gate 7 — Output Review | A4 HTML PDFs generated | Preview client documents; approve or reject | Teams + email always |
...
Before output generation, `openspec archive` runs: BSE calls `openspec archive {engagement-id}`. Spec files from `openspec/changes/` are merged into `openspec/specs/` as the permanent capability record in the client repo. This runs automatically after Gate 6 approval and before `outputGenerationChain`.
...
2. **Final Client Brief PDF** — refined post-spec summary. Generated at Gate 7. Client-facing. No scorecard data.
...
- **Tier 2 — BA retry:** if all auto-retries fail, Gate 7 is blocked at `gate7_review`. BA notified via Power Automate Teams with error and link. BA triggers retry from the Gate 7 review screen without rerunning full output generation. Each BA retry increments `engagements.project_summary_upload_attempts`.
...
### Table: engagements [v5.1 CHANGE] [v5.4 CHANGE]
...
### Table: gate_approvals [v5.1 CHANGE] [v5.4 CHANGE]
...
flagged_sections      text[]             -- capability folder names flagged for regeneration at Gate 5
...
### 10.5 Business Proposal Generation Prompt [v5.1 NEW]

Pipeline chain: `proposalGenerationChain`. Produces the A4 HTML business proposal sent to the client at Gate 3.
...
- **Business Proposal PDF** — problem brief + solution options. Generated at Gate 3. Client-facing. Delivered by Power Automate email to `client_email`. No internal pipeline data.
- **Final Client Brief PDF** — generated at Gate 7 for formal post-engagement delivery. Client-facing. No scorecard data.
...
Business Solutions/
  └── [ClientName]/
        └── [YYYY]/
              ...
              ├── [ClientName]_[YYYY-MM-DD]_Brief.pdf              ← Gate 7
              ├── [ClientName]_[YYYY-MM-DD]_Proposal.pdf           ← Gate 7
              ├── [ClientName]_[YYYY-MM-DD]_ProjectSummary.pdf     ← Gate 7 (internal) [v5.2]
              └── _internal/
...
| Gate 7 approved | `gate_approvals`: `gate_number = 7`, `action = 'approved'` | Teams + email confirmation with SharePoint links |
| Gate 7 rejected | `gate_approvals`: `gate_number = 7`, `action = 'rejected'` | Teams + email alert to BA |
...
POWER_AUTOMATE_GATE7_TRIGGER_URL=
...
**Schema additions first:** [v5.4 CHANGE]
...
- Extend `engagements.status` with `'proposal_pending'`, `'gate3_review'`, `'gate4_review'`, `'gate5_review'`, `'gate6_review'`, `'gate7_review'`, `'failed'` states [v5.4]
- Extend `gate_approvals.gate_number` check to cover `1–7` [v5.4]
...
- Implement `proposalGenerationChain` (produces A4 HTML → PDF business proposal) [v5.1]
- Build Gate 3 business proposal review screen — PDF preview, approve/reject, Send button [v5.1]
- Wire Power Automate Gate 3 proposal send flow [v5.1]
- Implement Gate 4 client decision screen — BA selects chosen solution from approved options (radio button); supplementary context capture via brain-dump, transcript, or guided questions (all optional); no-further-input checkbox; advances to spec_pending on approval [v5.4]
- Implement `contextGenerationChain` (produces CONTEXT.md + ADRs)
- Implement `openspecGenerationChain` (produces OpenSpec files in client repo)
- Implement Gate 5 spec review screen — renders OpenSpec markdown files; sections flaggable for regeneration
...
- Build Gate 6 code review screen — Gemini scorecard, ESLint CC scores per file (colour-coded), history, diff view, escalation flag [v5.4]
...
- Implement `openspec archive` step (runs after Gate 6 approval, before output generation)
...
- Implement business proposal PDF generation (used at Gate 3) [v5.1]
...
- Build Gate 7 review screen — client document preview, approve/reject
- Wire Power Automate Gate 7 flows (approval and rejection)
- End-to-end test including document generation and SharePoint filing

### Phase 5 — Power Platform & Refinement

- Build all Power Automate flows (intake notification, proposal send, CC pause notification, review report delivery, Gate 7 approval/rejection, chain failure alert) [v5.1]
...
code_pending
  → pre_check (lint + ESLint complexity + type-check + openspec validate)
    ...
    → if pre_check passes → code_review (Gemini scores)
      → if threshold_met → gate6_review (human)
      ...
      → if max_cycles reached → gate6_review (human, review_escalated = true)
```
...
Gate 5 approval = commit is authoritative
openspec archive runs after Gate 6 approval
Never store spec content in Supabase. The repo is the source of truth from Gate 5 onwards.
```
...
// gate_number = 6, action = 'cc_pause_approved' OR 'cc_pause_rejected'
...
React naming conventions. Tailwind class ordering. Supabase query patterns (`supabase-js` v2). Error handling and recovery pattern. Gate enforcement pattern (always server-side). Gate numbering reference (Gates 1–7) [v5.4]. Feature flag pattern. Model separation rule (Claude generates and fixes, Gemini reviews — never swap). Non-pipeline AI call labelling convention. OpenSpec file structure and WHEN/THEN/AND format. Fallow hook setup pattern. ESLint complexity rule configuration and CC severity classification. CC pause pattern.

**Skill 6 — OpenSpec + Matt Pocock Skills Reference**
OpenSpec BSE-custom schema definition. CLI commands used by BSE (`init`, `validate --all --json`, `archive`, `schema init`). Matt Pocock skills used in pipeline and their setup order: `/setup-matt-pocock-skills` (must run before `/to-issues`), `/git-guardrails`, `/to-issues`, `/tdd`, `/caveman`, `/zoom-out`. `/diagnose` as manual post-Gate-6 tool only — not in automated loop. Fallow hook installation and `gate=new-only` behaviour.
...
**Post Gate 6 — `fallow health` (manual, BA-invoked)**

After Gate 6 approval, the BA or client dev team can run `fallow health --format json` in the client repo to get an objective complexity and maintainability picture before merging. This surfaces file health scores, git churn hotspots, and ranked refactoring targets. It is not a pipeline gate — it is a decision support tool.
...
*Supersedes: v1.0, v2.0, v3.0, v4.0, v5.0, v5.1, v5.2, v5.3 and all pipeline planning notes*
