# Business Solutions Engine (BSE) — Full Project Instructions v5.5

**How to use this document** This is the complete operating manual for the BSE build project. You are a Claude instance in a dedicated build project. Your job is to implement the system described here. Read this document in full before writing any code. When in doubt, refer back here. This document is self-contained and supersedes all previous versions (v1.0, v2.0, v3.0, v4.0, v5.0, v5.1, v5.2, v5.3, v5.4) and all prior pipeline planning notes.

**What changed from v5.4** Sections 6.2, 6.3, 7.1, 7.3, 7.7, 7.8, 9, 10.5, 10.11 (new), 11, and 15 updated. Default analysis mode changed to Deep Analysis — Quick Ideas is now opt-in, the BA must explicitly select it. Document A (Solution Options Summary PDF) added as a new document generated automatically after Gate 2 approval. Gate 3 redesigned as the Client Decision, Proposal and Confirmation Loop — BA selects chosen solution, adds context, and reviews/edits Document B (Comotion-branded A4 proposal) in a loop until satisfied; rollback to Gate 2 available if client changes mind. `proposalEditChain` added for targeted proposal editing without full regeneration. Business Proposal generation prompt updated: now generates for a single chosen solution, not all options. New schema columns: `analysis_mode default 'deep'`, `chosen_solution_context`, `gate3_rollback_available`, `sharepoint_solution_options_url`. SharePoint folder structure updated with SolutionOptions file. All new changes marked [v5.5]. See the change log at the end of this document for a full summary.
...
### 6.2 Chains to Implement [v5.1 CHANGE] [v5.5 CHANGE]
...
### 6.3 Model Assignment [v5.5 CHANGE]
...
### 7.7 Human-in-the-Loop Gates [v5.1 CHANGE] [v5.4 CHANGE] [v5.5 CHANGE]
...
3. **Final Client Brief PDF** — refined post-spec summary. Generated at Gate 7. Client-facing. No scorecard data.
...
### Table: engagements [v5.1 CHANGE] [v5.4 CHANGE] [v5.5 CHANGE]
...
**Schema additions first:** [v5.4 CHANGE] [v5.5 CHANGE]
