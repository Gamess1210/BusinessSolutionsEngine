# Business Solutions Engine (BSE) — Full Project Instructions v5.8

**How to use this document** This is the complete operating manual for the BSE build project. You are a Claude instance in a dedicated build project. Your job is to implement the system described here. Read this document in full before writing any code. When in doubt, refer back here. This document is self-contained and supersedes all previous versions (v1.0, v2.0, v3.0, v4.0, v5.0, v5.1, v5.2, v5.3, v5.4, v5.5, v5.6, v5.7) and all prior pipeline planning notes.

**What changed from v5.7** Sections 5.2, 6.2, 7.1, 7.4, 7.5, 7.7, 9, 15, and 16 updated. A new `repo_setup` status is inserted between `gate5_review` and `spec_pending`. After Gate 5 approval, `gate5-approve-epic.js` now advances status to `repo_setup` (not `spec_pending`). BA connects the client GitHub repository, validates `.gitignore`, confirms skill files are uploaded, and triggers the scaffold commit — all at `repo_setup`. Only after the scaffold commit succeeds does status advance to `spec_pending`, where `contextGenerationChain` and `openspecGenerationChain` fire automatically as before. New API routes: `repo-validate.js` (GitHub repo validation + `.gitignore` fix), `repo-setup.js` (scaffold-only commit — CLAUDE.md, skills, OpenSpec config files — no chains), `skills-update.js` (BA updates skill file content). New Supabase columns on `engagements`: `github_repo text`, `eslint_config_exists boolean default false`. New `skills` table seeded with five rows. New pages: `RepoSetup.jsx` (`/review/:id/repo-setup`) and `Skills.jsx` (`/settings/skills`). See the change log at the end of this document for a full summary. [v5.8]
...
Capture inputs (any combination)
  ...
  Client Intake Form   ──┘
                    ...
                    └─────────┬──────────────────────────────────┘
                              │ [All epics approved → repo_setup]
                              ▼
                    ┌─────────▼──────────────────────────────────┐
                    │  REPO SETUP (BA action) [v5.8 NEW]         │ ◀── BA connects GitHub
                    │  BA enters org/repo-name; BSE validates    │     repo, confirms
                    │  GitHub access + .gitignore; BA confirms   │     skills uploaded,
                    │  skill files uploaded; BSE commits         │     triggers scaffold
                    │  scaffold to feature branch                │     commit
                    └─────────┬──────────────────────────────────┘
                              │ [Scaffold committed → spec_pending (automatic)]
                              ▼
...
### 6.2 Chains to Implement [v5.1 CHANGE] [v5.5 CHANGE] [v5.6 CHANGE] [v5.7 CHANGE] [v5.8 CHANGE]
...
**Gate 5 — three-phase process:** Gate 5 (Project Plan) is a three-phase process. Phase 1 runs `buildInstructionsChain` to produce `CLIENT_BUILD_INSTRUCTIONS.md`. Phase 2 uses `projectPlanChain` for epic discovery via iterative chat. Phase 3 uses `projectPlanChain` to generate stories and tasks for each approved epic. `current_plan_phase` on `engagements` tracks the active phase. After all epics are approved, status advances to `repo_setup` — not `spec_pending`. See Section 7.5 for full detail. [v5.6 NEW] [v5.7 REDESIGN] [v5.8 CHANGE]

**repo_setup — BA action before chains:** Between Gate 5 approval and `spec_pending`, the BA connects the client GitHub repository, validates GitHub access and `.gitignore`, confirms skill files are uploaded to the Skills Registry, and triggers the scaffold commit. No chain runs during `repo_setup`. After the scaffold commit succeeds, status advances to `spec_pending` automatically. [v5.8 NEW]

**spec_pending — chains fire automatically:** `contextGenerationChain` runs immediately on status = `spec_pending` — no BA action required. `openspecGenerationChain` follows. Both chains run automatically after the scaffold commit. [v5.8 NOTE]
...
- `/review/[id]/project-plan` — Gate 5 project plan review. Three-phase stepped interface: Phase 1 (build instructions review), Phase 2 (epic discovery chat), Phase 3 (per-epic story approval). [v5.6 NEW] [v5.7 REDESIGN]
- `/review/[id]/repo-setup` — Repository Setup. BA enters GitHub repo, validates access, confirms skill files, triggers scaffold commit to feature branch. Only accessible when status = `repo_setup`. [v5.8 NEW]
...
- `/settings/skills` — Skills Registry. Shows all five skill files with content preview, last-updated date, and upload button. Cards with null content are highlighted to indicate they block repo setup. [v5.8 NEW]
...
### 7.5 Gate 5 — Project Plan [v5.6 NEW] [v5.7 REDESIGN] [v5.8 CHANGE]
...
- `POST /api/pipeline/gate5-approve-epic` — validates `current_plan_phase === 3`; inserts `gate_approvals` (`gate_number: 5`, `action: 'epic_approved'`, epic title in `edits_made`); if all epics approved: stores `project_plan`, stores `plan_conversation`, advances `status` to `repo_setup` [v5.8 CHANGE — was `spec_pending`]
...
After all epics are approved, status advances to `repo_setup`. The BA connects the client GitHub repo, validates `.gitignore`, confirms skill files are uploaded, and triggers the scaffold commit at `/review/:id/repo-setup`. After the scaffold commit, status advances automatically to `spec_pending`, where `contextGenerationChain` runs once to produce `CONTEXT.md` and `docs/adr/` entries. Then `openspecGenerationChain` runs epic by epic — one capability folder per epic in the approved project plan. The BA reviews specs one epic at a time at Gate 6. [v5.8 CHANGE]

### 7.4 Gate 6 — Spec Review Detail [v5.6 CHANGE] [v5.8 CHANGE]
...
After the scaffold commit at `repo_setup`, status advances automatically to `spec_pending` and two chains run in sequence — no BA action required:

`contextGenerationChain` runs once at `spec_pending` — Claude reads the chosen solution, approved brief, all engagement inputs, and the approved project plan and produces `CONTEXT.md` (domain vocabulary, key terms, architectural decisions) and `docs/adr/` entries for the client repo. The scaffold files committed during `repo_setup` (CLAUDE.md, skill files, OpenSpec config) are already in the feature branch as context. [v5.8 CHANGE]
...
### 7.7 Error Recovery [v5.8 CHANGE]
...
**Note:** `repo_setup` is a BA-action status — no chain runs. It sits between `gate5_review` and `spec_pending`. The BA connects the GitHub repo, confirms the Skills Registry, and triggers the scaffold commit. `spec_pending` is the automatic chain phase — `contextGenerationChain` and `openspecGenerationChain` fire immediately on transition to `spec_pending`. [v5.8 NEW]

### 7.8 Human-in-the-Loop Gates [v5.1 CHANGE] [v5.4 CHANGE] [v5.5 CHANGE] [v5.6 CHANGE] [v5.7 CHANGE]
...
| Gate 5 — Project Plan | Gate 4 approved; status = plan_pending | Three-phase process: Phase 1 — review and approve CLIENT_BUILD_INSTRUCTIONS.md; Phase 2 — iterative chat to define and approve epic list; Phase 3 — per-epic story and task review and approval. All three phases at `/review/:id/project-plan`. On final epic approval, status advances to `repo_setup`. [v5.7 REDESIGN] | None |
...
### Table: engagements [v5.1 CHANGE] [v5.4 CHANGE] [v5.5 CHANGE] [v5.6 CHANGE] [v5.8 CHANGE]
...
status                text default 'captured' check (status in (
                        ...
                        'gate4_review',
                        'plan_pending', 'gate5_review',
                        'repo_setup',                              -- [v5.8 NEW] BA connects repo + scaffold commit
...
github_repo           text               -- org/repo-name format, set during repo_setup [v5.8]
eslint_config_exists  boolean default false  -- true if repo already has an ESLint config; skips eslint.config.js scaffold [v5.8]
...
### Table: skills [v5.8 NEW]

```sql
-- Stores BSE skill file content (SKILL.md files) for the Skills Registry.
-- Five rows seeded at migration time; content is null until BA uploads.
-- Null content on any row blocks repo_setup with a 422 error.

id           uuid primary key default gen_random_uuid()
name         text not null unique  -- 'bse-prompt-library', 'bse-schema-reference',
                                   -- 'bse-langchain-patterns', 'bse-component-standards',
                                   -- 'bse-openspec-skills-reference'
folder_path  text not null         -- path: .agents/skills/{name}/SKILL.md
content      text                  -- null until BA uploads; null blocks repo_setup
updated_by   uuid references auth.users(id)
updated_at   timestamptz default now()
created_at   timestamptz default now()
```

RLS: authenticated users can SELECT; INSERT/UPDATE via service role only.

Seed rows (content = null):

| name | folder_path |
|---|---|
| bse-prompt-library | .agents/skills/bse-prompt-library/SKILL.md |
| bse-schema-reference | .agents/skills/bse-schema-reference/SKILL.md |
| bse-langchain-patterns | .agents/skills/bse-langchain-patterns/SKILL.md |
| bse-component-standards | .agents/skills/bse-component-standards/SKILL.md |
| bse-openspec-skills-reference | .agents/skills/bse-openspec-skills-reference/SKILL.md |
...
- Add `repo_setup` to `engagements.status` check constraint (between `gate5_review` and `spec_pending`) [v5.8]
- Add `github_repo text` and `eslint_config_exists boolean default false` columns to `engagements` [v5.8]
- Create `skills` table with five seeded rows (`content = null`): `bse-prompt-library`, `bse-schema-reference`, `bse-langchain-patterns`, `bse-component-standards`, `bse-openspec-skills-reference` [v5.8]
- Add RLS on `skills`: authenticated users SELECT; INSERT/UPDATE via service role only [v5.8]
...
- Implement `buildInstructionsChain` (generates CLIENT_BUILD_INSTRUCTIONS.md from structured_brief, chosen_solution, engagement_inputs, industry — Phase 1 of Gate 5) [v5.7]
- Implement `projectPlanChain` (three-phase orchestrator: Phase 1 build instructions, Phase 2 epic discovery, Phase 3 per-epic story/task generation; stores approved plan in `engagements.project_plan`) [v5.6] [v5.7 REDESIGN]
- Build Gate 5 project plan review screen (`/review/:id/project-plan`) — three-phase stepped interface; phase indicator (1/2/3); Phase 1: build instructions review; Phase 2: epic discovery chat; Phase 3: per-epic story approval [v5.6] [v5.7 REDESIGN]
- Implement six Gate 5 API routes [v5.7 — replace v5.6 routes]: `plan-build-instructions`, `gate5-approve-instructions`, `plan-discover-epics`, `gate5-approve-epics`, `plan-generate-epic-stories`, `gate5-approve-epic` (final: advances status to `repo_setup`)
- Implement `/api/pipeline/repo-validate.js` — validates GitHub repo access, checks `.gitignore` for `.agents/`, returns `repoName`, `defaultBranch`, `proposedBranch`, `gitignoreOk`; handles `action: 'fix-gitignore'` to append `.agents/` via GitHub Contents API; writes `github_repo` and `eslint_config_exists` to `engagements` on successful validation [v5.8]
- Implement `/api/pipeline/repo-setup.js` — scaffold-only commit: CLAUDE.md (template fill), five skill files from `skills` table, `openspec/config.yaml`, `openspec/changes/{id}/.openspec.yaml`, `openspec/changes/{id}/proposal.md`, `openspec/changes/{id}/tasks.md`, conditional `eslint.config.js`; uses GitHub Git Data API for atomic branch creation + single commit; advances `status` to `spec_pending` on success; no chains called [v5.8]
- Implement `/api/settings/skills-update.js` — upserts skill content by `skillName`; validates skill name exists in `skills` table; returns updated skill record [v5.8]
- Build `RepoSetup.jsx` (`/review/:id/repo-setup`) — Step 1: repo validation; Step 2: folder structure preview + "Create Structure" trigger; redirects to `/engagements/:id` if status ≠ `repo_setup` [v5.8]
- Build `Skills.jsx` (`/settings/skills`) — five skill cards with upload button, null-content highlight, content preview, "View full content" toggle [v5.8]
...
### Skills Registry Pattern [v5.8 NEW]

The `skills` table holds five SKILL.md files used in the client repo scaffold. Content is managed via `/settings/skills` (BA uploads `.md` file; stored as text in `skills.content`). Null content on any row blocks `repo-setup.js` with HTTP 422 before any GitHub call.

```javascript
// repo-setup.js — precondition check
const { data: skills } = await supabase.from('skills').select('name, content')
const missing = skills.filter(s => !s.content).map(s => s.name)
if (missing.length > 0) {
  return res.status(422).json({ error: 'Missing skill content', missingSkills: missing })
}

// Each skill committed as: .agents/skills/{name}/SKILL.md
// Content from skills.content — no chain call
// Labelled: // NON-PIPELINE: template fill
```

Skill content changes apply to new engagements only. Existing feature branches are not retroactively updated.

### .gitignore Enforcement Pattern [v5.8 NEW]

`repo-validate.js` checks the client repo's `.gitignore` for the `.agents/` entry before allowing the BA to proceed to scaffold creation. If `.agents/` is absent, the RepoSetup screen shows a hard-block warning and a "Fix .gitignore" button.

```javascript
// repo-validate.js — gitignore check
// GET /repos/{owner}/{repo}/contents/.gitignore
// Decode base64 content; check for '.agents/' or '.agents'
// Return: { gitignoreOk: boolean, gitignoreMissing: boolean }

// action: 'fix-gitignore'
// PUT /repos/{owner}/{repo}/contents/.gitignore
// Append '.agents/' to existing content
// Return: { fixed: true, commitSha }
```

The BA cannot proceed to Step 2 (folder structure preview) until `gitignoreOk === true`. This prevents BSE-generated agent skill files from being committed to version control in client repos.
...
Full table definitions from Section 9 including all new tables and extended columns. Include RLS policies. Note that `specifications` uses `repo_path` + `commit_sha` — not `spec_content` JSONB. Note new columns: `client_email` on `engagements`; `sharepoint_proposal_url` on `engagements`; `eslint_cc_report` and `cc_pause_occurred` on `code_reviews`; `cc_pauses` on `review_loop_reports`; `chosen_solution` and `gate4_no_further_input` on `engagements` [v5.4]; `chosen_solution_context`, `gate3_rollback_available`, and `sharepoint_solution_options_url` on `engagements` [v5.5]; `analysis_mode` default changed to `'deep'` [v5.5]; `project_plan`, `plan_conversation`, and `current_epic_index` on `engagements` [v5.6]; `build_instructions`, `approved_epics`, and `current_plan_phase` on `engagements` [v5.7]; `github_repo` and `eslint_config_exists` on `engagements` [v5.8]; `repo_setup` added to `status` constraint [v5.8]. New `skills` table (five seeded rows, null content blocks repo_setup) [v5.8]. Note that `gate_approvals.gate_number` now covers 1–8 and `action` includes `'sent'`, `'plan_approved'`, `'instructions_approved'`, `'epics_approved'`, `'epic_approved'`, `'cc_pause_approved'`, `'cc_pause_rejected'`. Refer to this before writing any database query.

**Skill 3 — LangChain Chain Patterns**
LangChain 0.3.x patterns for Claude (`@langchain/anthropic`) and Gemini (`@langchain/google-genai`). Chain composition pattern. Input/output typing. Error handling. Retry logic. Model separation rule. Include `proposalGenerationChain` pattern (now takes chosen solution + additional context + brief). Include `proposalEditChain` pattern (targeted edits only — does not rewrite). Include `buildInstructionsChain` pattern (Phase 1 — generates CLIENT_BUILD_INSTRUCTIONS.md from structured_brief, chosen_solution, engagement_inputs, industry) [v5.7]. Include `projectPlanChain` pattern (three-phase: Phase 1 build instructions, Phase 2 epic discovery via iterative chat, Phase 3 per-epic story/task generation) [v5.7 REDESIGN]. Include the ESLint pre-check stage pattern with CC severity classification. Include the CC 21+ pause/resume pattern. Include the `openspec archive` step. Reference before implementing any chain.
...
| 3 | 7.4 | Gate 6 (formerly Gate 5) — Spec Approval updated. Specs are now generated epic by epic from the approved project plan. Each epic becomes one capability folder. `contextGenerationChain` runs once after Gate 5 approval; `openspecGenerationChain` runs per epic. BA reviews specs one epic at a time. Gate 6 approval records are per-epic. **Note: as of v5.8, chain generation happens during `spec_pending` (after Repository Setup), not immediately after Gate 5 approval.** | Gate redesign |
...
Merged from Geoff's v5.7 Gate 5 redesign (base) + repository setup additions. One conflict resolved: `gate5-approve-epic.js` previously advanced status to `spec_pending`; v5.8 changes this to `repo_setup`. See entry 1 below.

| # | Section affected | Change | Type |
|---|---|---|---|
| 1 | 7.5, 5.2, 7.7 | **CONFLICT RESOLUTION** — `gate5-approve-epic.js` final epic approval now advances `status` to `repo_setup` (not `spec_pending`). A BA action step at `repo_setup` precedes the automatic chain phase at `spec_pending`. Geoff's v5.7 had `spec_pending` direct; our v5.7 added `repo_setup` between them. v5.8 resolves this in favour of the repo_setup design. | Conflict resolution |
| 2 | 5.2, 7.7, 9 | New `repo_setup` status added to state machine between `gate5_review` and `spec_pending`. `repo_setup` is a BA-action status — no chain runs. Status advances to `spec_pending` automatically after scaffold commit. State machine note added in 7.7. | New status |
| 3 | 7.1 | Two new pages added: `/review/:id/repo-setup` (RepoSetup.jsx — Step 1: repo validation; Step 2: folder structure preview + scaffold commit trigger) and `/settings/skills` (Skills.jsx — Skills Registry management). | New pages |
| 4 | 7.4 | "What happens before Gate 6" updated: chains now run at `spec_pending`, not immediately after Gate 5 approval. Scaffold files committed during `repo_setup` are already in the feature branch as context when chains run. | Architecture note |
| 5 | 6.2 | Three notes added: (a) Gate 5 final approval advances to `repo_setup` not `spec_pending`; (b) `repo_setup` is a BA-action status — no chain; (c) `spec_pending` is the automatic chain phase — `contextGenerationChain` fires immediately on transition. | Architecture note |
| 6 | 9 (engagements) | Two new columns: `github_repo text` (set during repo_setup validation) and `eslint_config_exists boolean default false` (set during repo_setup validation; controls whether `eslint.config.js` is included in scaffold). `repo_setup` added to `status` check constraint. | Schema addition |
| 7 | 9 (skills table) | New `skills` table added. Five rows seeded at migration time with `content = null`. BA uploads SKILL.md content via `/settings/skills`. Null content on any row blocks `repo-setup.js` with HTTP 422. RLS: SELECT for authenticated users; INSERT/UPDATE via service role only. | New table |
| 8 | 15 | Schema additions listed: `repo_setup` in status constraint, `github_repo`, `eslint_config_exists`, `skills` table + RLS + seed rows. New routes in Phase 3 build list: `repo-validate.js`, `repo-setup.js`, `skills-update.js`. New pages in build list: `RepoSetup.jsx`, `Skills.jsx`. | Build sequence update |
| 9 | 16 | Two new patterns added: Skills Registry Pattern (null-content gate, per-skill file commit to `.agents/skills/{name}/SKILL.md`, `// NON-PIPELINE: template fill` label) and .gitignore Enforcement Pattern (hard-block on missing `.agents/` entry; "Fix .gitignore" button appends via GitHub Contents API). | New patterns |
