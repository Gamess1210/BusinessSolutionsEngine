    ...
    <div style="border-bottom:2pt solid ${COLORS.navy};padding-bottom:10pt;margin-bottom:16pt;">
      <div style="display:flex;align-items:flex-start;justify-content:space-between;">
        <div>
          ...
          <p style="font-size:10pt;color:${COLORS.grey};margin:3pt 0 0 0;">To-Be Process Map</p>
...
function renderSteps(steps) {
  ...
  const rows = steps.map(s => `
    <tr>
      ...
      <td style="font-size:9pt;color:${COLORS.green};padding:6pt 0;border-bottom:1pt solid ${COLORS.greyMid};width:20%;vertical-align:top;font-style:italic;">${esc(s.improvements ?? '')}</td>
    </tr>`).join('')
  const header = `<tr>${th('#')}${th('Step')}${th('Description')}${th('Actor')}${th('System')}${th('Improvement')}</tr>`
  return `
    <div style="margin-bottom:14pt;">
      ${sectionHeading('Future-State Process Steps')}
...
function renderComparison(items) {
  ...
  const rows = items.map(c => `
    <tr>
      <td style="font-size:9pt;font-weight:600;color:${COLORS.navy};padding:5pt 8pt 5pt 0;border-bottom:1pt solid ${COLORS.greyMid};width:20%;vertical-align:top;">${esc(c.process_area ?? '')}</td>
      <td style="font-size:9pt;color:#374151;padding:5pt 8pt 5pt 0;border-bottom:1pt solid ${COLORS.greyMid};width:30%;vertical-align:top;">${esc(c.as_is ?? '')}</td>
      <td style="font-size:9pt;color:#374151;padding:5pt 8pt 5pt 0;border-bottom:1pt solid ${COLORS.greyMid};width:30%;vertical-align:top;">${esc(c.to_be ?? '')}</td>
      <td style="font-size:9pt;color:${COLORS.green};padding:5pt 0;border-bottom:1pt solid ${COLORS.greyMid};width:20%;vertical-align:top;font-weight:600;">${esc(c.improvement ?? '')}</td>
    </tr>`).join('')
  const header = `<tr>${th('Process Area')}${th('As-Is')}${th('To-Be')}${th('Improvement')}</tr>`
  return `
    <div style="margin-bottom:14pt;">
      ${sectionHeading('As-Is vs To-Be Comparison')}
      <table style="width:100%;border-collapse:collapse;"><thead>${header}</thead><tbody>${rows}</tbody></table>
    </div>`
}

function renderKpiImprovements(items) {
  if (!items?.length) return ''
  const rows = items.map(k => `
    <tr>
      <td style="font-size:9pt;font-weight:600;color:${COLORS.navy};padding:5pt 8pt 5pt 0;border-bottom:1pt solid ${COLORS.greyMid};width:28%;vertical-align:top;">${esc(k.metric ?? '')}</td>
      <td style="font-size:9pt;color:#374151;padding:5pt 8pt 5pt 0;border-bottom:1pt solid ${COLORS.greyMid};width:22%;vertical-align:top;">${esc(k.current_value ?? '')}</td>
      <td style="font-size:9pt;color:#374151;padding:5pt 8pt 5pt 0;border-bottom:1pt solid ${COLORS.greyMid};width:22%;vertical-align:top;">${esc(k.target_value ?? '')}</td>
      <td style="font-size:9pt;color:${COLORS.green};padding:5pt 0;border-bottom:1pt solid ${COLORS.greyMid};width:28%;font-weight:600;vertical-align:top;">${esc(k.improvement_pct ?? '')}</td>
    </tr>`).join('')
  const header = `<tr>${th('KPI')}${th('Current')}${th('Target')}${th('Expected Improvement')}</tr>`
  return `
    <div style="margin-bottom:14pt;">
      ${sectionHeading('KPI Improvement Targets')}
      <table style="width:100%;border-collapse:collapse;"><thead>${header}</thead><tbody>${rows}</tbody></table>
    </div>`
}
...
function renderFooter(doc) {
  return `
    <div style="position:absolute;bottom:18px;left:40px;right:40px;border-top:1pt solid ${COLORS.greyMid};padding-top:6pt;display:flex;justify-content:space-between;align-items:center;">
      ...
      <span style="font-size:8pt;color:${COLORS.grey};">${esc(doc.client_name ?? '')} — To-Be Process Map</span>
    </div>`
}

export function renderToBeProcessMapHtml(doc) {
  return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8" />
...
<div class="page">
  ...
  ${renderKpiImprovements(doc.kpi_improvements)}
  ${renderFooter(doc)}
</div>
</body>
</html>`
