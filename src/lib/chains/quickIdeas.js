...
async function parseJsonWithFallback(message) {
  try {
    ...
    if (start === -1) throw new Error('quickIdeasChain: failed to parse Claude response as JSON')
    const closeChar = stripped[start] === '{' ? '}' : ']'
    const end = stripped.lastIndexOf(closeChar)
    if (end === -1) throw new Error('quickIdeasChain: failed to parse Claude response as JSON')
    try {
