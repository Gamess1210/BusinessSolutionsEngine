...
export default async function handler(req, res) {
  ...
  const { engagementId, chosenSolution } = req.body ?? {}
  if (!engagementId || !chosenSolution) {
    return res.status(400).json({ error: 'engagementId and chosenSolution are required' })
  }
  ...
  try {
    const { error } = await supabaseAdmin
      .from('engagements')
      .update({ chosen_solution: chosenSolution, gate3_rollback_available: true })
      .eq('id', engagementId)
    if (error) throw error
    return res.status(200).json({ success: true, engagementId })
  } catch (err) {
    console.error(`[BSE] gate3-select-solution failed for ${engagementId}:`, err.message)
    return res.status(500).json({ error: 'Failed to persist chosen solution' })
  }
}
